...
#import "NSString+BFPinYinObjC.h"

@interface BFPinyinFormatter ()
+(NSInteger)getNumericValue:(unichar)c;
+(NSInteger)indexOfChar:(int*) table ch:(unichar)c;
@end    

@implementation BFPinyinFormatter
...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
  ...
  if ((BFToneTypeWithToneMark == [outputFormat toneType]) && ((BFVCharTypeWithV == [outputFormat vCharType]) || (BFVCharTypeWithUAndColon == [outputFormat vCharType]))) {
  ...
  if (BFToneTypeWithoutTone == [outputFormat toneType]) {
  ...
  else if (BFToneTypeWithToneMark == [outputFormat toneType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"v"];
     pinyinStr = [BFPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
  }
  if (BFVCharTypeWithV == [outputFormat vCharType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"v"];
  }
  else if (BFVCharTypeWithUUnicode == [outputFormat vCharType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"ü"];
  }
  if (BFCaseTypeUppercase == [outputFormat caseType]) {
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
        int tuneNumber = [BFPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
