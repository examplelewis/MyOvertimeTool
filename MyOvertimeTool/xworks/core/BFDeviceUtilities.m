...
UIInterfaceOrientation BFInterfaceOrientation(void) {
    ...
    BFASSERT(UIDeviceOrientationUnknown != orient);
    
    return orient;
}

BOOL BFIsLandscapePhoneOrientation(UIInterfaceOrientation orientation) {
    return BFIsPhone() && UIInterfaceOrientationIsLandscape(orientation);
}
...
Hardware BFHardware() {
    NSString *hardware = BFHardwareString();
...
float BFHardwareNumber(Hardware hardware) {
    switch (hardware) {
...
BOOL BFIsCurrentDeviceHardwareBetterThan(Hardware otherHardware) {
    float otherHardwareNumber = BFHardwareNumber(otherHardware);
    float currentHardwareNumber = BFHardwareNumber(BFHardware());
    return currentHardwareNumber >= otherHardwareNumber;
}
...
CGSize backCameraStillImageResolutionInPixels()
{
    switch (BFHardware()) {
        ...
        default:
            ...
            NSLog(@"Your device is: %@", BFHardwareDescription());
            break;
    }
    return CGSizeZero;
}
