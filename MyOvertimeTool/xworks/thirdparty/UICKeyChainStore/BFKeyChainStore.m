...
@interface BFKeyChainStore () {
    NSMutableDictionary *itemsToUpdate;
}

@end

@implementation BFKeyChainStore
...
{
    return [BFKeyChainStore removeItemForKey:key service:[self defaultService] accessGroup:nil];
}

+ (BOOL)removeItemForKey:(NSString *)key service:(NSString *)service
{
    return [BFKeyChainStore removeItemForKey:key service:service accessGroup:nil];
}
...
{
    NSArray *items = [BFKeyChainStore itemsForService:service accessGroup:accessGroup];
...
{
    NSArray *items = [BFKeyChainStore itemsForService:self.service accessGroup:self.accessGroup];
