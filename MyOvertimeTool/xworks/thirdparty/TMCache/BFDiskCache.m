...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath
{
    ...
    if (self = [super init]) {
        _name = [name copy];
        _queue = [BFDiskCache sharedQueue];
        ...
        __weak BFDiskCache *weakSelf = self;

        dispatch_async(_queue, ^{
            BFDiskCache *strongSelf = weakSelf;
...
+(BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSURL *uniqueTrashURL = [[BFDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString];
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    BOOL trashed = [BFDiskCache moveItemAtURLToTrash:fileURL];
    if (!trashed)
        return NO;
    
    [BFDiskCache emptyTrash];
...
- (void)objectForKey:(NSString *)key block:(BFDiskCacheObjectBlock)block
{
...
- (void)fileURLForKey:(NSString *)key block:(BFDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(BFDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(BFDiskCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToSize:(NSUInteger)trimByteCount block:(BFDiskCacheBlock)block
{
    if (trimByteCount == 0) {
...
- (void)trimToDate:(NSDate *)trimDate block:(BFDiskCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToSizeByDate:(NSUInteger)trimByteCount block:(BFDiskCacheBlock)block
{
    if (trimByteCount == 0) {
        [self removeAllObjects:block];
        return;
...
- (void)removeAllObjects:(BFDiskCacheBlock)block
{
    TMCacheStartBackgroundTask();
    ...
    dispatch_async(_queue, ^{
        ...
        [BFDiskCache moveItemAtURLToTrash:strongSelf->_cacheURL];
        [BFDiskCache emptyTrash];
...
- (void)enumerateObjectsWithBlock:(BFDiskCacheObjectBlock)block completionBlock:(BFDiskCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id <NSCoding>)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(BFDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        objectForKey = object;
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(BFDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key
{
    ...
    [self setObject:object forKey:key block:^(BFDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(BFDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSize:(NSUInteger)byteCount
{
    ...
    [self trimToSize:byteCount block:^(BFDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(BFDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSizeByDate:(NSUInteger)byteCount
{
    ...
    [self trimToSizeByDate:byteCount block:^(BFDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(BFDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(BFDiskCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(BFDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (BFDiskCacheObjectBlock)willAddObjectBlock
{
    __block BFDiskCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(BFDiskCacheObjectBlock)block
{
    __weak BFDiskCache *weakSelf = self;
...
- (BFDiskCacheObjectBlock)willRemoveObjectBlock
{
    __block BFDiskCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(BFDiskCacheObjectBlock)block
{
    __weak BFDiskCache *weakSelf = self;
...
- (BFDiskCacheBlock)willRemoveAllObjectsBlock
{
    __block BFDiskCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(BFDiskCacheBlock)block
{
    __weak BFDiskCache *weakSelf = self;
...
- (BFDiskCacheObjectBlock)didAddObjectBlock
{
    __block BFDiskCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(BFDiskCacheObjectBlock)block
{
    __weak BFDiskCache *weakSelf = self;
...
- (BFDiskCacheObjectBlock)didRemoveObjectBlock
{
    __block BFDiskCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(BFDiskCacheObjectBlock)block
{
    __weak BFDiskCache *weakSelf = self;
...
- (BFDiskCacheBlock)didRemoveAllObjectsBlock
{
    __block BFDiskCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(BFDiskCacheBlock)block
{
    __weak BFDiskCache *weakSelf = self;
