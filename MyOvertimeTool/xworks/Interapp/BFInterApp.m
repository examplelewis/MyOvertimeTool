...
+ (BOOL)openPreferredBrowserWithURL:(NSURL *)url {
    if (sPreferGoogleChrome && [BFInterApp googleChromeIsInstalled]) {
        return [BFInterApp googleChromeWithURL:url];
    } else {
        return [BFInterApp safariWithURL:url];
    }
}
...
+ (BOOL)openBestGoogleMapUrl:(NSString*)urlString{
    if ([BFInterApp googleMapsIsInstalled]) {
        ...
        return [BFInterApp openPreferredBrowserWithURL:url];
    }
}
...
+ (BOOL)googleMapAtLocation:(CLLocationCoordinate2D)location {
    ...
    return [BFInterApp openBestGoogleMapUrl:urlPath];
}
...
+ (BOOL)googleMapAtLocation:(CLLocationCoordinate2D)location title:(NSString *)title {
    NSString* urlPath = [NSString stringWithFormat:@"?q=%@@%f,%f",
                            BFStringByAddingPercentEscapesForURLParameterString(title),
...
+ (BOOL)googleMapDirectionsFromLocation:(CLLocationCoordinate2D)fromLocation
    ...
    return [BFInterApp googleMapDirectionsFromLocation:fromLocation toLocation:toLocation withMode:nil];
}
...
+ (BOOL)googleMapDirectionsFromLocation:(CLLocationCoordinate2D)fromLocation
    ...
    return [BFInterApp googleMapDirectionsFromSourceAddress:saddr toDestAddress:daddr withMode:directionsMode];
}

+ (BOOL)googleMapDirectionsToLocation:(CLLocationCoordinate2D)toLocation
                             withMode:(NSString *)directionsMode {
    NSString* daddr = [NSString stringWithFormat:@"%f,%f", toLocation.latitude, toLocation.longitude];
    return [BFInterApp googleMapDirectionsFromSourceAddress:nil toDestAddress:daddr withMode:directionsMode];
}

+ (BOOL)googleMapDirectionsToDestAddress:(NSString *)destAddr withMode:(NSString *)directionsMode {
    return [BFInterApp googleMapDirectionsFromSourceAddress:nil toDestAddress:destAddr withMode:directionsMode];
}
...
+ (BOOL)googleMapWithQuery:(NSString *)query {
    NSString* urlPath = [NSString stringWithFormat:@"?q=%@", BFStringByAddingPercentEscapesForURLParameterString(query)];
    return [BFInterApp openBestGoogleMapUrl:urlPath];
}
...
+ (BOOL)mailWithInvocation:(BFMailAppInvocation *)invocation {
    ...
    if (BFIsStringWithAnyText(invocation.recipient)) {
        urlPath = [urlPath stringByAppendingString:BFStringByAddingPercentEscapesForURLParameterString(invocation.recipient)];
    }
    
    if (BFIsStringWithAnyText(invocation.cc)) {
        [parameters setObject:invocation.cc forKey:@"cc"];
    }
    if (BFIsStringWithAnyText(invocation.bcc)) {
        [parameters setObject:invocation.bcc forKey:@"bcc"];
    }
    if (BFIsStringWithAnyText(invocation.subject)) {
        [parameters setObject:invocation.subject forKey:@"subject"];
    }
    if (BFIsStringWithAnyText(invocation.body)) {
        [parameters setObject:invocation.body forKey:@"body"];
    }
    
    urlPath = BFStringByAddingQueryDictionaryToString(urlPath, parameters);
...
+ (NSURL *)urlForInstagramImageAtFilePath:(NSString *)filePath error:(NSError **)error {
    ...
    if (image.size.width < 612
        ...
        if (nil != error) {
            *error = [NSError errorWithDomain: BFNimbusErrorDomain
                                         code: BFImageTooSmall
                                     userInfo: [NSDictionary dictionaryWithObject: image
                                                                           forKey: BFImageErrorKey]];
        }
        return nil;
    }
    ...
    BFASSERT(BFIsArrayWithObjects(paths));
    if (!BFIsArrayWithObjects(paths)) {
        return nil;
    }
...
@implementation BFMailAppInvocation
